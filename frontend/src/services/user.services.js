...
const userUpdateProfileDetails = async (id, email, fullname, dob, location) => {
  return api
    .patch(`/api/user/update/${id}/personal-details`, {
      email,
      fullname,
      dob,
      location,
...
const userDeleteAccount = async (id, reason) => {
  return api
    .patch(`/api/user/update/${id}/delete-account`, {
      reason,
    })
    .then((response) => {
      if (response.status === 200) {
...
const userMediaGenderUpload = async (id, profileImage, video, gender) => {
  ...
  return await apiFormData
    .patch(`/api/user/media/${id}`, formData, {
